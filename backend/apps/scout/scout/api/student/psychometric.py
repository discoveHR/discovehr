...
from scout.api.psychometric.tao_client import create_student_delivery_session, psychometric_dev_mode_enabled


@frappe.whitelist(methods=["GET"])
def list_psychometric_assignments():
    ...
    rows = frappe.get_all(
        ...
        fields=["name", "psychometric_assessment", "student_user", "status",
    ...
    assessment_ids = list({r["psychometric_assessment"] for r in rows if r.get("psychometric_assessment")})
    ...
    results_map = {
        ...
        for r in frappe.get_all(
            ...
            fields=["name", "assignment", "overall_score", "scores_json",
                    "traits_json", "recommendations", "completed_at", "psychometric_assessment"],
        )
    }

    assignments = []
    for row in rows:
        assessment = assessments_map.get(row.get("psychometric_assessment") or "")
...
def launch_psychometric_assignment():
    user_id, err = get_student_session_user()
    if err:
        return err
    ...
    assignment = frappe.get_doc("Scout Psychometric Assignment", assignment_id)
    ...
    assessment = frappe.get_doc("Scout Psychometric Assessment", assignment.psychometric_assessment)
    ...
    if not assessment.tao_test_id:
        if psychometric_dev_mode_enabled():
            assessment.tao_test_id = f"dev-{assessment.name}"
    ...
    return {
        ...
        "data": {
            ...
            "launchUrl": assignment.launch_url,
            "devMode": session.get("mode") == "dev" or psychometric_dev_mode_enabled(),
        },
    }


@frappe.whitelist(methods=["POST"])
def submit_psychometric_dev_result():
    """Local completion when TAO is not configured (dev/demo)."""
    user_id, err = get_student_session_user()
    if err:
        return err
    if not psychometric_dev_mode_enabled():
        frappe.local.response["http_status_code"] = 403
        return {"ok": False, "message": _("Dev psychometric mode is disabled.")}
    ...
    sample = {
        "overallScore": 72.5,
        "scores": {"openness": 78, "conscientiousness": 70, "extraversion": 65, "agreeableness": 80, "stability": 69},
        "traits": {
            "primary": "Analytical Collaborator",
            "secondary": "Structured Learner",
        },
        "recommendations": _(
            "Strong fit for roles requiring teamwork and structured problem solving. "
            "Consider communication-heavy campus activities to balance profile."
        ),
        "completedAt": now_datetime(),
    }
    result = apply_psychometric_result(assignment_name=assignment_id, payload=sample, source="dev")
    return {"ok": True, "message": result.get("message"), "data": {"assignmentId": assignment_id}}


@frappe.whitelist(methods=["GET"])
def get_psychometric_result():
    user_id, err = get_student_session_user()
    if err:
        return err
    ...
    result_name = frappe.db.get_value("Scout Psychometric Result", {"assignment": assignment_id}, "name")
    if not result_name:
        frappe.local.response["http_status_code"] = 404
        return {"ok": False, "message": _("No result yet.")}

    row = frappe.get_doc("Scout Psychometric Result", result_name).as_dict()
    return {
        "ok": True,
        "data": {
