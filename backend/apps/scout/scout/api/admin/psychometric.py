...
from scout.api.psychometric.results import row_to_assignment
from scout.api.psychometric.tao_client import create_psychometric_test_in_tao
...
def _row_to_assessment(row: dict) -> dict:
    return {
        ...
        "assignmentCount": frappe.db.count("Scout Psychometric Assignment", {"psychometric_assessment": row.get("name")}),
    }


@frappe.whitelist(methods=["GET"])
def list_psychometric_assessments():
...
def create_psychometric_assessment():
    user_id, err = get_admin_session_user()
    if err:
        return err
    ...
    if manual_tao_id:
        ...
        tao_sync = create_psychometric_test_in_tao(doc)
    ...
    return {
        "ok": True,
        "message": _("Psychometric assessment created."),
        "data": {"assessment": _row_to_assessment(doc.as_dict())},
    }


def _resolve_student_emails(payload: dict) -> list[str]:
    """Merge explicit emails, student ids, and optional whole-college roster."""
    emails: set[str] = set()
    ...
    *,
    ...
    for raw_email in student_emails:
        ...
        existing = frappe.db.exists(
            ...
            {
                "psychometric_assessment": assessment_id,
                "student_user": email,
        ...
        doc = frappe.get_doc(
            {
                "doctype": "Scout Psychometric Assignment",
                "psychometric_assessment": assessment_id,
                "student_user": email,
...
@frappe.whitelist(methods=["POST"])
def assign_psychometric_to_students():
    user_id, err = get_admin_session_user()
    if err:
        return err
    ...
    if not assessment_id or not frappe.db.exists("Scout Psychometric Assessment", assessment_id):
        frappe.local.response["http_status_code"] = 400
    ...
    assessment = frappe.get_doc("Scout Psychometric Assessment", assessment_id)
...
def list_psychometric_assignments():
    ...
    filters = {}
    if assessment_id:
        filters["psychometric_assessment"] = assessment_id
