...
import {
  assignAptitudeToStudents,
  createAdminAptitudeAssessment,
  getAdminCollegesDirectory,
  getAdminCollegeStudents,
  getAptitudeModuleConfig,
  listAdminAptitudeAssessments,
  listAdminAptitudeAssignments,
  simulateAptitudeWebhook,
...
export function AdminAptitudePanel() {
  ...
  async function refresh(assessmentId?: string) {
    ...
    try {
      const [assessmentRows, assignmentRows, moduleConfig] = await Promise.all([
        listAdminAptitudeAssessments(),
        listAdminAptitudeAssignments(assessmentId),
        getAptitudeModuleConfig(),
      ]);
      ...
      setError(err instanceof Error ? err.message : "Failed to load aptitude data.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function handleCreate(event: FormEvent) {
    ...
    setMessage("");
    try {
      const created = await createAdminAptitudeAssessment({
        title: title.trim(),
  ...
  async function handleAssign(event: FormEvent) {
    ...
    setMessage("");
    try {
      const result = await assignAptitudeToStudents({
  ...
  async function handleSimulate(assignmentId: string) {
    ...
    setError("");
    try {
      const res = await simulateAptitudeWebhook(assignmentId);
      ...
      <div className="tpo-panel">
        ...
        <form className="job-form-grid" onSubmit={handleCreate}>
          <div className="job-form-row">
            <label>Title</label>
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Campus aptitude — 2026" required />
          </div>
          <div className="job-form-row">
        ...
        <table className="company-table">
          ...
          <tbody>
            {assessments.length === 0 ? (
              <tr>
                <td colSpan={4}>No aptitude assessments yet.</td>
              </tr>
            ) : (
              assessments.map((a) => (
                <tr key={a.id}>
